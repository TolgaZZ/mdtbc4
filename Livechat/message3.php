...
if (is_numeric($_POST['channel'])) {
    ...
    $query = "SELECT * FROM livechat WHERE channel = {$_POST['channel']} AND user_id = {$_SESSION['user_id']} OR channel = {$_SESSION['user_id']} AND user_id = {$_POST['channel']} ORDER BY created_at DESC LIMIT 10";
